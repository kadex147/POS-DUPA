...
<div id="postTransactionDetailModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 w-full max-w-md relative">
        ...
        <h3 class="text-xl font-bold mb-4 text-green-600">Transaksi Berhasil!</h3> 
        
        <div class="mb-4 space-y-1 text-sm text-gray-600">
            <div class="flex justify-between">
                <span class="font-semibold">Invoice:</span>
                <span id="postDetailModalInvoice" class="font-mono"></span>
            </div>
            <div class="flex justify-between">
        ...
        <div class="border-t pt-4 mb-4">
            <div class="flex justify-between font-bold text-lg">
                <span>Total:</span>
                <span id="postDetailModalTotal">Rp 0</span>
            </div>
        </div>
...
function confirmOrder() {
    ...
    // --- BAGIAN INI DIUBAH ---
    .then(data => {
        if (data.success && data.transaction) { // Pastikan data.transaction ada
            closeOrderModal();   // Tutup modal konfirmasi dulu
            showPostTransactionModal(data.transaction); // Tampilkan modal detail sukses
            
            // Kosongkan keranjang setelah modal ditampilkan
            cart = [];
            saveCart();
            updateCart();
            
        } else if (data.success) {
             // Fallback jika data.transaction tidak ada
             alert('Transaksi berhasil! Nomor Invoice: ' + data.invoice_number);
             cart = [];
             saveCart();
             updateCart();
             closeOrderModal();
        } else {
            alert('Transaksi gagal: ' + (data.message || 'Terjadi kesalahan tidak diketahui.'));
        }
    })
    // --- BATAS PERUBAHAN ---
    .catch(error => {
        // Tampilkan pesan error yang lebih detail di console untuk debugging
...
// --- FUNGSI BARU UNTUK MODAL SETELAH SUKSES ---
function showPostTransactionModal(transactionData) {
    const data = transactionData; // Ambil data transaksi dari argumen
    ...
    // Isi modal
    document.getElementById('postDetailModalInvoice').textContent = data.invoice_number;
    document.getElementById('postDetailModalTanggal').textContent = formattedDate;
    document.getElementById('postDetailModalKasir').textContent = data.user.username; 
...
function closePostTransactionModal() {
    ...
    modal.classList.add('hidden');
    modal.classList.remove('flex');
    // Anda bisa tambahkan aksi lain di sini jika perlu, misal:
