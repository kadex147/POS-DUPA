{{-- resources/views/components/transaction-success-modal.blade.php --}}
...
<div id="postTransactionDetailModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg shadow-xl p-4 lg:p-6 w-full max-w-md relative max-h-[90vh] overflow-y-auto">
        ...
        <!-- Tombol Aksi -->
        <div class="space-y-2">
            <button onclick="printReceiptFromSuccessModal()" 
...
<script>
// Global variable untuk menyimpan data transaksi sukses
window.currentSuccessTransaction = null;
...
window.showPostTransactionModal = function(transactionData) {
    ...
    window.currentSuccessTransaction = transactionData;
...
/**
 * Mencetak struk dari modal sukses transaksi
 */
window.printReceiptFromSuccessModal = function() {
    if (!window.currentSuccessTransaction) {
        alert('Data transaksi tidak tersedia');
        return;
    }

    const transaction = window.currentSuccessTransaction;
